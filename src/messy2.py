...
class StatisticalModel:
    def __init__(self, highest_order=2, nb_l=2, nb_u=4, poly_order=4, tree_depth=2, binary_operators = [sp.Mul], unary_functions = pow):
    ...
    def Multi_Level_SDE(self, X, N_iters=10, threshold_sample=1e-1, poly_bases=False, verbose = False, piecewise=False):
      flag = False
      x = self.x
      while not flag:
        Nt = 20
    ...
    def CrossEntropy(self, X,H,H_new,f_g,f_g_s,Nm_xe,list_mass,list_pdf,poly_bases=False, piecewise=False):
      x = self.x
      done = False
      n_m = Nm_xe
      while not done:
    ...
    def get_pdf(self, X, N_iters=5, Nm_xe=2, N_levels=3, threshold_sample=1e-1, verbose=False, piecewise=False):
      min_rel_err, min_rel_err_h = np.inf, np.inf
      dic = {}
      i = 0
      R_low = [self.x**i for i in range(1, Nm_xe+1)]
      R_low_ = [sp.lambdify(self.x,r_s,"numpy") for r_s in R_low]
      X_moms_low = self.moments(R_low_, X)
      while i < N_iters:
        #if verbose:
        print('Round:', i, " out of ", N_iters, "\n")
        f_g, f_g_s, list_mass, list_pdf = self.Multi_Level_SDE(X, N_levels, threshold_sample, poly_bases=(i==0), verbose=verbose, piecewise=piecewise)
        print('SDE done, Starting MxED')
        f_g_new, f_g_s_new, samples, weights, corrector = self.CrossEntropy(X,R_low, R_low_,f_g,f_g_s,Nm_xe,list_mass,list_pdf,poly_bases=(i==0),piecewise=piecewise)
        print('MxED done')
